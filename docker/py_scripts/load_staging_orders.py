...
def load_staging_orders():
    try:
        ...
        insert_log(conn_dest, "load_staging_orders", "Running", start_time=start_time)
        ...
        cur_dest.execute("TRUNCATE TABLE restaurant.staging_orders")

        # Fetch data from restaurant_db and insert into restaurant_dwh
        cur_source.execute("SELECT order_id, order_date, order_time FROM restaurant.orders")
        ...
        for row in rows:
            order_id, order_date, order_time = row
            cur_dest.execute("INSERT INTO restaurant.staging_orders (order_id, order_date, order_time, created_date, created_time) VALUES (%s, %s, %s, CURRENT_DATE, CURRENT_TIME)",
                             (order_id, order_date, order_time))
        
        # Log successful completion
        end_time = datetime.now()
        insert_log(conn_dest, "load_staging_orders", "Success", start_time=start_time, end_time=end_time, records_processed=records_processed)   
        ...
        end_time = datetime.now()
        insert_log(conn_dest, "load_staging_orders", "Failure", start_time=start_time, end_time=end_time, error_message=str(e))
        print("Error:", e)
